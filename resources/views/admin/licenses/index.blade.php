<x-admin-layout>
        ...
        <div class="licenses-container">
            ...
            <div class="page-header-premium">
                <div class="header-main">
                    <h1 class="premium-title">Driver License Management</h1>
                    <div class="header-line"></div>
                </div>
                <p class="premium-subtitle">Track and update driving licenses for all registered drivers.</p>
            </div>

            <!-- STATS OVERVIEW -->
            <div class="stats-row mt-6">
                <div class="mini-stat-card">
                    <span class="mini-label">Total Drivers</span>
                    <span class="mini-value">{{ $drivers->count() }}</span>
                </div>
                <div class="mini-stat-card">
                    <span class="mini-label">Valid Licenses</span>
                    <span class="mini-value text-green">{{ $drivers->where('license_status', 'ok')->count() }}</span>
                </div>
                <div class="mini-stat-card">
                    <span class="mini-label">Renewal Due</span>
                    <span class="mini-value text-gold">{{ $drivers->whereIn('license_status', ['warning', 'critical'])->count() }}</span>
                </div>
                <div class="mini-stat-card">
                    <span class="mini-label">Expired</span>
                    <span class="mini-value text-red">{{ $drivers->where('license_status', 'expired')->count() }}</span>
                </div>
            </div>
            ...
            <div class="premium-card mt-8">
                <div class="card-header-flex">
                    <h3 class="card-title">Driver License Status</h3>
                </div>
                        ...
                        <tbody>
                            @forelse($drivers as $driver)
                                <tr>
                                    ...
                                    <td><code class="license-code">{{ $driver->license_number ?? 'N/A' }}</code></td>
                                    <td>
                                        <span class="{{ $driver->license_status == 'expired' ? 'text-red font-bold' : ($driver->license_status == 'critical' ? 'text-orange font-bold' : '') }}">
                                            {{ $driver->license_expiry ? \Carbon\Carbon::parse($driver->license_expiry)->format('d M, Y') : 'Not Set' }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="status-badge {{ $driver->license_status }}">
                                            {{ ucfirst($driver->license_status) }}
                                        </span>
                                    </td>
                                    <td>
                                        <a href="{{ route('admin.licenses.edit', $driver->id) }}" class="btn-action edit" title="Update License">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-8">No drivers found in the system.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <style>
        ...
        .licenses-page-wrapper { padding: 10px 20px 40px; }
        .licenses-container { max-width: 1200px; margin: 0 auto; }
        ...
        .driver-cell i { color: var(--maroon); font-size: 16px; }
        
        .license-code { background: #f1f5f9; padding: 4px 8px; border-radius: 6px; font-family: 'Monaco', monospace; font-size: 12px; color: var(--maroon); }
