        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="welcome-header-minimal mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="page-title">Edit Receipt: {{ $receipt->receipt_no }}</h1>
                        <p class="text-sm text-gray-500 mt-1">Update the official payment record details below.</p>
                    </div>
                ...
                <form action="{{ route('admin.receipts.update', $receipt->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div>
                            ...
                            <input type="text" name="customer_name" id="customer_name" class="premium-input" value="{{ $receipt->customer_name }}" required>
                        </div>
                        <div>
                            <label for="customer_phone" class="premium-label">Phone Number</label>
                            <input type="text" name="customer_phone" id="customer_phone" class="premium-input" value="{{ $receipt->customer_phone }}" required>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div>
                            ...
                            <select name="bus_number" id="bus_number" class="premium-input" required>
                                ...
                                @foreach($buses as $bus)
                                    <option value="{{ $bus->plate_number }}" {{ $receipt->bus_number == $bus->plate_number ? 'selected' : '' }}>
                                        {{ $bus->plate_number }} 
                        ...
                        <div>
                            ...
                            <input type="text" name="trip_route" id="trip_route" class="premium-input" value="{{ $receipt->trip_route }}" required>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div>
                            ...
                            <div class="amount-input-wrap">
                                ...
                                <input type="number" name="amount" id="amount" class="premium-input has-prefix" value="{{ $receipt->amount }}" step="0.01" required>
                            </div>
                        </div>
                        <div>
                            ...
                            <select name="payment_method" id="payment_method" class="premium-input" required>
                                <option value="M-Pesa" {{ $receipt->payment_method == 'M-Pesa' ? 'selected' : '' }}>M-Pesa</option>
                        ...
                        <div>
                            ...
                            <input type="date" name="receipt_date" id="receipt_date" class="premium-input" 
                                value="{{ $receipt->receipt_date->format('Y-m-d') }}" 
                                max="{{ date('Y-m-d') }}" 
                                required>
                        </div>
                    </div>
                    ...
                    <div class="form-actions mt-10">
                        <button type="submit" class="btn-update-receipt">
                            <i class="fas fa-save mr-2"></i> Update Receipt Record
                        </button>
                    </div>
                </form>
            </div>
